...
function getEarlyBirdsCount(): number {
	const count = parseInt(import.meta.env.VITE_COUNT_EARLY_BIRDS, 10);
	return count;
}

function getFreeTrialsCount(): number {
	const count = parseInt(import.meta.env.VITE_COUNT_FREE_TRIALS, 10);
	return count;
}

function getFullSubscribersCount(): number {
	const count = parseInt(import.meta.env.VITE_COUNT_FULL_SUBSCRIBERS, 10);
	return count;
}
